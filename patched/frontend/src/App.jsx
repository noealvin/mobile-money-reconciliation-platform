...
export default function App() {
  ...
  const runReconciliation = async () => {
    try {
      const response = await axios.post("http://127.0.0.1:8000/reconciliation/run");
  ...
  const resolveAnomaly = async (id) => {
    ...
    setResolvingId(id);
    try {
      await axios.put(`http://127.0.0.1:8000/anomalies/${id}/resolve`);
  ...
  // FILTERED ANOMALIES
  // =========================
  const allAnomalies = reconciliation?.anomalies || [];
  ...
  return (
    <div style={{
            ...
            {profileOpen && (
              <div style={{
                ...
                <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                  <p style={{ fontWeight: 600, fontSize: 14, margin: 0 }}>Admin</p>
                  <p style={{ fontSize: 12, color: "#64748b", margin: 0 }}>admin@mmrecon.com</p>
                </div>
                <button style={dropItemStyle}>👤 Mon profil</button>
                <button style={dropItemStyle}>⚙️ Paramètres</button>
                <button style={{ ...dropItemStyle, borderTop: "1px solid rgba(255,255,255,0.07)", color: "#f87171", marginTop: 4 }}>
                  🚪 Déconnexion
                </button>
              </div>
            )}
      ...
      <div style={{ padding: "32px", maxWidth: 1300, margin: "0 auto" }}>
        ...
        ========================== */}
        <h2 style={sectionTitleStyle}>Vue d'ensemble — aujourd'hui</h2>
            ...
            {anomaliesOpen && (
              <div style={{
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
                  {[
                    { key: "all",      label: `Tous statuts`, color: "#94a3b8" },
                    { key: "OPEN",     label: `Ouvertes (${countByStatus("OPEN")})`, color: "#ef4444" },
                    { key: "RESOLVED", label: `Résolues (${countByStatus("RESOLVED")})`, color: "#22c55e" },
                  ].map(f => (
                    <button
                      key={f.key}
                      onClick={() => { setStatusFilter(f.key); setAnomalyPage(1); }}
                      style={chipStyle(statusFilter === f.key, f.color)}
                    >{f.label}</button>
                  ))}
                </div>
